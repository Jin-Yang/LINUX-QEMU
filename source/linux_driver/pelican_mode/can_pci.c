...
static void send_message(void __iomem *base, struct can_frame *can)
{
	...
	temp |= DLC(can->can_dlc);
	if (can->can_id & (1 << 30)) // RTR
		temp |= RTR;
	if (can->can_id & (1 << 31)) { // EFF
		temp |= EFF;
		iowrite8(temp, base + index); // Write to TX frame information.
		index++;
		
		temp = (can->can_id >> 21) & 0xff;
		iowrite8(temp, base + index);
		index++;
		temp = (can->can_id >> 13) & 0xff;
		iowrite8(temp, base + index);
		index++;
		temp = (can->can_id >> 5) & 0xff;
		iowrite8(temp, base + index);
		index++;
		temp = (can->can_id & 0x1f) << 0x03;
		iowrite8(temp, base + index);
		index++;
...
static void receive_message(void __iomem *base, struct can_frame *can)
{
	...
	if (temp & (1 << 7)) { // EFF
		can->can_id |= (1 << 31);
		temp = ioread8(base + index++); // CAN ID
		can->can_id |= ((temp << 21) & (0xff << 21));
		temp = ioread8(base + index++);
		can->can_id |= ((temp << 13) & (0xff << 13));
		temp = ioread8(base + index++);
		can->can_id |= ((temp <<  5) & (0xff <<  5));
		temp = ioread8(base + index++);
		can->can_id |= ((temp >>  3) & 0x1f);
	} else { // SFF
		temp = ioread8(base + index++); // CAN ID
	...
	for(i = 0; i < can->can_dlc; i++)
   	...
   	iowrite8(0x04, base + SJA_COMMAND_REG);// Send transmission request.
}
...
static irqreturn_t irq_handler(int irq, void *dev)
{
	...
	if (temp & RI) {
		DPRINTF("   Receive int in(%d) out(%d) cnt(%d)\n", 
...
int device_open( struct inode *node, struct file *filp )
{
	...
	// Filter set
	temp = ioread8(device->mem_base + SJA_MODE_REG);
	iowrite8(temp | (1 << 3), device->mem_base + SJA_MODE_REG);

	for(count = 20; count < 24; count++)
		iowrite8(0xff, device->mem_base + count);
	

	// Enable transmit interrupt
	temp = ioread8(device->mem_base + SJA_INT_EN_REG);
	iowrite8(temp | TIE | RIE, device->mem_base + SJA_INT_EN_REG);
